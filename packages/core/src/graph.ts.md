...
describe('collectModuleSpecifiers', () => {
  it('collects imports, re-exports, and dynamic imports from TypeScript AST', () => {
    expect(
      collectModuleSpecifiers([
        "import { a } from ':a'",
        "export { b } from ':b'",
...
describe('detectCycle', () => {
  it('detects no cycle', () => {
    ...
    documents.set('/a.ts.md', { main: "import './b.ts.md'" });
    documents.set('/b.ts.md', { main: 'export const value = 1' });
    expect(detectCycle('/a.ts.md:main', (file) => documents.get(file))).toBeNull();
  });

  it('detects a cycle between modules', () => {
    const documents = new Map<string, Record<string, string>>();
