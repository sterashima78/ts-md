# Compiling a TypeScript project that contains TS-MD documents

`ts-md-tsc` adapts the TypeScript compiler rather than replacing it. A TS-MD document is expanded into virtual TypeScript source files, those files participate in the ordinary TypeScript program, and emitted paths and diagnostics are translated back to the document-facing representation.

The implementation follows five stages:

1. Parse the command line and include `.ts.md` as a mixed-content extension.
2. Build a document store that gives every code fence a stable virtual filename.
3. Present those virtual files through a wrapped `CompilerHost`.
4. Rewrite emitted filenames, module specifiers, and source-map references so virtual identities do not leak into published output.
5. Map diagnostics back to offsets in the Markdown document.

The plain `index.ts` file remains the stage-zero bootstrap. Everything that understands TS-MD lives here, while process startup stays compilable by ordinary TypeScript.
